<script lang="ts">
	...
	import type { OneRimInfo, OrderConfig, RimConfig } from "../../types";
    ...
    const sendOrderToApi = async (name: string, phone: string, email: string, config: RimConfig, url: string, rimId: string) => {
        let requestData:{
            name: string, phone: string, email: string, orderConfig: OrderConfig,
        };
    ...
    <div class="modal">
        <div class="modalCard">
            ...
            <div class="content">
                ...
                <button aria-label="submit-button" aria-labelledby="submit" class="orderBtn" on:click={orderRim}>Оформить заказ</button>
            </div>
        </div>
    </div>
    <button class="cancelBth" on:click={clickOutside}>Отменить</button>
{/if}
    ...
    <div class="modalTXH">
        <div class="modalCardTHX">
            <div class="modalHeaderTXH">
                <img class="logo" src={logoIcon} alt="Ukrdisk">
