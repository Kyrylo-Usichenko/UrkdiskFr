			...
			<div class="searchResults">
				{#each searchResults as result}
					<a
						class="searchResult"
						target="_blank"
						href={`/rim?id=${result.rimId}&diameter=${result.config[0].diameter}&width=${result.config[0].width}&pcd=${result.config[0].boltPattern}`}
					>
						<div class="suggestedResult">
							...
							<div class="suggestedInfo">
								...
								<p class="modelPrice">
									{result.minPrice[0] > 0 ? `от ${result.minPrice[0]}` : 'Нет в наличии'}
								</p>
							</div>
						</div>
					</a>
...
<section class="rimMakers">
	...
	<div class="makers">
		...
		<button
			class={`showMoreMakers ${!showMoreMakers ? '' : 'hiden'}`}
			on:click={() => {
				showMoreMakers = true;
			}}>Показать еще 2</button
		>
